...
if [ $mode == "train" ]; then
  ...
  python train.py $bin_path --save-dir $cp_path --tensorboard-logdir $cp_path --seed 555 --num-workers 2 \
         ...
         --optimizer adam --adam-betas "(0.9, 0.98)" --lr 5e-4 --lr-scheduler inverse_sqrt --warmup-updates 500 \
         --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --no-epoch-checkpoints \
         --max-tokens 100 --update-freq 1 --validate-interval 1 --patience 10 \
