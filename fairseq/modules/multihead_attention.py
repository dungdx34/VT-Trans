...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_memory:
            mem_length = 2
            self.q_proj = quant_noise(nn.Linear(self.qdim * mem_length, embed_dim, bias=bias), q_noise, qn_block_size)
    ...
    def forward(
        ...
        if self.use_memory:
            ...
            #print(query.size())
            #print(key.size())
            #query = query.reshape(-1, 16384)
            #query = query.reshape(-1, 4096)
            query = query.reshape(-1, 2048)
