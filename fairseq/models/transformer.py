...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        start_ctxlayer = args.encoder_layers - 2
        #start_ctxlayer = args.encoder_layers - 4
        self.layers.extend(
    ...
    def forward(self, src_tokens, src_lengths, return_all_hiddens: bool = False):
        ...
        # encoder layers
        attn = []
        mems = torch.randn(x.size(0), x.size(1), x.size(2)).to(x.device)
        nn.init.normal_(mems)
        mems = nn.Parameter(mems)
        for layer in self.layers:
            x, layer_attn, mems = layer(mems, x, padding_mask, local_attn_mask=local_attn_mask)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.self_partial_mode = True
        self.cross_partial_mode = True
        #######################
        self.dropout = args.dropout
        ...
        start_dec_ctxlayer = args.decoder_layers - 2
        #start_dec_ctxlayer = args.decoder_layers - 4
        # start_crs_ctxlayer = args.decoder_layers - args.cross_ctxlayers
        start_crs_ctxlayer = args.decoder_layers - 2
        #start_crs_ctxlayer = args.decoder_layers - 4
        self.layers.extend(
            [
    ...
    def extract_features_scriptable(
        ...
        mems = torch.randn(encoder_out.encoder_out.size(0), encoder_out.encoder_out.size(1), encoder_out.encoder_out.size(2)).to(x.device)
        nn.init.normal_(mems)
        mems = nn.Parameter(mems)
        for idx, layer in enumerate(self.layers):
            x, layer_attn, _, mems = layer(
                mems,
                x,
