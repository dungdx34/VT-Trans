...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadAttention2
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, add_global_attn=False):
        ...
        self.self_attn_memory = self.build_self_attention_memory(self.embed_dim, args.decoder_attention_heads, args)
        #######################
        ...
        self.fc_memory = self.build_fc1(
            self.embed_dim*16, self.embed_dim, self.quant_noise, self.quant_noise_block_size
        )

        self.final_layer_norm = LayerNorm(self.embed_dim)

        # self.seg_embedding = SinusoidalPositionalEmbedding(
        #     self.mem_length) if pos_type == "sinusoidal" else LearnablePositionalEmbedding(self.mem_length)

        self.seg_embedding = SinusoidalPositionalEmbedding(16)
        # self.seg_embedding = LearnablePositionalEmbedding(16)
    ...
    def build_self_attention_memory(self, embed_dim, attn_heads, args):
        return MultiheadAttention(
            embed_dim,
            attn_heads,
    ...
    def forward(self, mems, x, padding_mask, local_attn_mask: Optional[Tensor] = None):
        """
        Args:
        ...
        #x, attn_global = self.self_attn_local(
        #        query=x,
        #        key=x,
        #        value=x,
        #        key_padding_mask=padding_mask,
        ...
        residual2 = x
        new_mems = self.seg_embedding(mems)
        x_mem, attn_mem = self.self_attn_memory(
            query=new_mems,
            key=x,
            value=x,
            static_kv=True,
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.fc_memory = self.build_fc1(
            self.embed_dim * 8, self.embed_dim, self.quant_noise, self.quant_noise_block_size
        )
    ...
    def forward(
        ...
        if self.encoder_attn_local is not None:
            ...
            #x, attn_global = self.encoder_attn_local(
            #        query=x,
            #        key=encoder_out,
            #        value=encoder_out,
            #        key_padding_mask=encoder_padding_mask,
